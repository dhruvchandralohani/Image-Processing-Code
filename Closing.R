library(imager)
img1 <- load.image("/home/dhruv/Desktop/Original_Image(Opening_&_Closing).png")
img1 <- grayscale(img1)
wid <- width(img1)
hei <- height(img1)
mat <- as.matrix(img1)
...
for(i in ceiling(size[1]/2):endd1){
  for(j in ceiling(size[1]/2):endd2){
    for(k in 1:size[1]){
      for(l in 1:size[1]){
        new_mat2[i,j] <- new_mat2[i,j] + calc_mat[i+vx[l]+1,j+vy[k]+1]*struc_ele[val+vx[l],val+vy[k]]
      }
    }
    if(new_mat2[i,j] == x){    #If the result of correlation is equal to the value of x i.e. total 1's in SE
